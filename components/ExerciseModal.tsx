  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white p-6 shadow-xl">
        <div className="mb-4 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              {exercise.category} - {exercise.region.join(", ")}
            </p>
          </div>
          <button onClick={onClose} className="rounded-xl bg-slate-200 px-3 py-1 text-sm font-bold">
            Close
          </button>
        </div>

        <Detail title="Dosage" items={[`${exercise.dosage.sets} sets x ${exercise.dosage.reps}, ${exercise.dosage.frequency}`]} />
        ...
        <Detail title="Precautions" items={exercise.precautions} />
      </div>
    </div>
  );
}
